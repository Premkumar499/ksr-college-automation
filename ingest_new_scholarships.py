...
Helpful Videos:
How to Register on NSP OTR: https://www.youtube.com/watch?v=IH3oESZjGV4
How to Apply for NSP Scholarship (Fresh Application): https://youtu.be/UFKaWaQm17M?si=ED6brQh-bav1qFlD
Registration Link:
NSP Portal Application: https://scholarships.gov.in/
...
def chunk_data():
    ...
    # Note: genai client handles API key from env variable natively if not specified directly or use genai.embed_content
    # Since heavy_server uses genai.embed_content globally, we do it too.
    
    texts = [d["content"] for d in documents]
    metadatas = [{"source": d["source"], "scholarship_name": d["scholarship_name"], "doc_type": d["doc_type"]} for d in documents]

    try:
        vector_db = ChromaWrapper(path=settings.CHROMA_GEMINI_DB_PATH)
        vector_db.add(documents=texts, metadatas=metadatas)
        print(f"Successfully added {len(documents)} chunks to Chroma DB!")
    except Exception as e:
        print(f"Error during embedding generation or database ingestion: {e}")
